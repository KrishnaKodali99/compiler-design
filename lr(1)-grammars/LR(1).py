...
NT, T, NT_set, T_set, first = list(), list(), set(), set(), dict()
...
if __name__ == "__main__":
    ...
    StartState()
    LR_1()
    print("The states of the LR(1) Automaton of the grammar are: ")
    for i in range(len(States)):
        S = States[i]
        print("State " + str(i))
        for j in range(len(S)): 
        ...
        print("\n")
    print("To show DFA transitions type 1")
    k = int(input())
    if(k == 1):
        LR1_Automaton()
    print("To show LR(1) ParseTable type 1")
    k = int(input())
    if(k == 1):
        df = LR1_ParseTable()
        if("S'" in df.columns):
